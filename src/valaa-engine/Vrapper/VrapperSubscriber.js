...
import { isNativeIdentifier, getNativeIdentifierValue } from "~/valaa-script";

import Cog from "~/valaa-engine/Cog";
import Vrapper from "~/valaa-engine/Vrapper";
import VALEK, { Kuery, dumpKuery, dumpObject } from "~/valaa-engine/VALEK";
import FieldUpdate from "~/valaa-engine/Vrapper/FieldUpdate";

import SimpleData from "~/valaa-tools/SimpleData";

import { invariantify, invariantifyObject, thenChainEagerly, wrapError } from "~/valaa-tools";
...
export default class VrapperSubscriber extends SimpleData {
  ...
  _sendUpdate (fieldUpdate: FieldUpdate) {
    try {
      thenChainEagerly(fieldUpdate.value(), (value) => {
