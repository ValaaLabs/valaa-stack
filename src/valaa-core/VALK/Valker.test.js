...
import { created, modified, destroyed } from "~/valaa-core/command";
import VALK from "~/valaa-core/VALK";
...
import GhostPath from "~/valaa-core/tools/denormalized/GhostPath";
...
describe("The snapshot node walker", () => {
  ...
  it("retrieves aliased value properly", async () => {
    const harness = createCoreTestHarness({ debug: 0 }, createBlockA, createBlockARest);
...
describe("ghost lookups", () => {
  ...
  it("Resource-to-Immaterial-Transient - if an Instance Prototype has a field reference to its " +
    ...
    const harness = createCoreTestHarness({ debug: 0 }, createTestObj, createTestObjInst);
  ...
  it("Immaterial-Transient-to-* - The ghost grandling should be immaterial", () => {
    const harness = createCoreTestHarness({ debug: 0 }, createTestObj, createOwnlingInst);
...
describe("mutations", () => {
  ...
  it("MODIFIED setting a field on a Transient should materialize it", () => {
    const harness = createCoreTestHarness({ debug: 0 }, createData, createInstance);
  ...
  it("MODIFIED remove on a plural field should cause changes to be reflected on Instances", () => {
    const harness = createCoreTestHarness({ debug: 0 }, createData, extraData,
        createInstance);
  ...
  it("MODIFIED commands on a Prototype should reflect as new field values on all Instances and" +
    ...
    const harness = createCoreTestHarness({ debug: 0 }, createData, createBlankInstance);
...
describe("complex structures", () => {
  ...
  */
  ...
  function createAndExtractHarnessWithCommands (options, ...commandLists) {
    harness = createCoreTestHarness(options, baseData,
