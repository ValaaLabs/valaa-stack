import { created, modified } from "~/valaa-core/command";
import VALK from "~/valaa-core/VALK";

import { createCoreTestHarness } from "~/valaa-core/test/CoreTestHarness";
import { VRef, IdData, vRef, getRawIdFrom } from "~/valaa-core/ValaaReference";

import { createTransient } from "~/valaa-core/tools/denormalized/Transient";
import { createMaterializeGhostAction, createImmaterializeGhostAction, isGhost, isMaterialized,
    createGhostVRefInInstance } from "~/valaa-core/tools/denormalized/ghost";
...
function setUp ({ debug, commands = [] }: any) {
  harness = createCoreTestHarness({ debug }, createBaseTestObjects, createRootInstance, commands);
}
