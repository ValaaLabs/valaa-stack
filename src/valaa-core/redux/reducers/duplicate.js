...
import { derivedId, dumpify, dumpObject, invariantify, wrapError } from "~/valaa-tools";
import {
  DuplicateBard,
...
function _duplicateOwnlingField (bard: Bard, fieldIntro: Object, originalIdData: any,
  ...
  try {
    ...
    if (typeof recombineOverriddenId !== "undefined") {
      if (bard.logger) {
        bard.logger.log(`${bard.getName()}: virtually recombining to sub-directive ${
    ...
    if (bard.objectTransient) {
      ...
      if (bard.logger) {
        bard.logger.log(`${bard.getName()}: Sub-reducing virtual DUPLICATED ${
